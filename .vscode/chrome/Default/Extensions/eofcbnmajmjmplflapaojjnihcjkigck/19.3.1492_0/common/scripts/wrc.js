...
(function (_, AvastWRC, PROTO) {
    ...
    AvastWRC.local = AvastWRC.local || {
         ...
         */
        connect: function () {
            if(_connectAvastTimer !== null){
                clearInterval(_connectAvastTimer);
                _connectAvastTimer = null;
            }
            _connectAvastTimer = setInterval(this.recheckConnectionAndUpdataData.bind(this), CHECK_LOCAL_PORT);
            
            AvastWRC.local.updatePortAndToken(null, null);
            AvastWRC.local.setGuids("", "", "", "");

            return new Promise((resolve, reject)=>{
                if(AvastWRC.CONFIG.LOCAL_ENABLED) {
                    var promisePorts = [];
                    if(!AvastWRC.Query.CONST.LOCAL_PORT){ //no connection established yet
                        //Iterate over possible localhost ports to find the one Avast listening on
                        AvastWRC.Query.CONST.LOCAL_PORTS.forEach(port => {
                            promisePorts.push(this.connectAvastCheck(port));
                        });                        
                    }
                    Promise.all(promisePorts).then(() => {
                        if(AvastWRC.Query.CONST.LOCAL_PORT){
                            var pairedData = [];
                            pairedData.push(this.getGuids());
                            pairedData.push(this.getSourceId());
                            Promise.all(pairedData).then((results) => {
                                this.updatePairedData(results);
                                resolve(); 
        ...
        updatePairedData: function(data){
            data.forEach(v => {
                if(v.key === "GUIDS"){
                    if(v.value[0].length > 0){
                        AvastWRC.local.setGuids(v.value[0], v.value[1], v.value[2], v.value[3]);
                    }                    
                }
                else if(v.key === "DISTRIBUTION_ID"){
                    AvastWRC.local.setSourceId(v.value);
                }
            });
        },
        recheckConnectionAndUpdataData: function () {// no promise this is only to update the data automatically     
            console.log("-----------------Reconecting AVAST and geting GUIDs----------------------------");
            var promisePorts = [];
            //Iterate over possible localhost ports to find the one Avast listening on
            AvastWRC.Query.CONST.LOCAL_PORTS.forEach(port => {
                promisePorts.push(this.connectAvastCheck(port));
            ...
            return new Promise((resolve, reject)=>{
                new AvastWRC.Query.Avast({
                    ...
                    callback: function (r) {
                        ...
                        if (r.values_ && r.values_.result && 
                             (PROTO.decodeUTF8(r.values_.result[0]) === "Avast" || PROTO.decodeUTF8(r.values_.result[0]) === "AVG" ) && 
                             r.values_.result.length > 2) {
                            var token = r.values_.result[3] ? PROTO.decodeUTF8(r.values_.result[3]) : null;
                            AvastWRC.local.updatePortAndToken(port, token);
                            resolve();
                        } 
                        else {
                            resolve();
        ...
        updatePortAndToken: function(activePort, activeToken){
            AvastWRC.Query.CONST.LOCAL_PORT = activePort;
            AvastWRC.Query.CONST.LOCAL_TOKEN = activeToken;
        },
            ...
            return new Promise((resolve, reject)=>{
                new AvastWRC.Query.Avast({
                    ...
                    callback: function (r) {
                        ...
                        console.log("GET_GUIDS: ", decoded);
                        resolve({key: "GUIDS", value: decoded});
                    },
                    error: function (e) {
                        var def =["", "", "", ""];
                        resolve({key: "GUIDS", value: def});
                    }
                });
            });
        },

        setGuids: function(guid, auid, hwid, uuid) {
            AvastWRC.avastConfig.load();

            if (guid !=="" ) AvastWRC.CONFIG.GUID = guid;
            if (auid !== "") AvastWRC.CONFIG.AUID = auid;
            if (hwid !== "") AvastWRC.CONFIG.HWID = hwid;
        ...
        getSourceId: function () {
            return new Promise((resolve, reject)=>{
                new AvastWRC.Query.Avast({
                    type: "GET_BCU_DISTRIBUTION_ID",
                    callback: function (r) {
                        var decoded = _.map(r.values_.result, function(v) { return v ? PROTO.decodeUTF8(v) : v; } );
                        resolve({key: "DISTRIBUTION_ID", value: decoded});
                    },
                    error: function (e) {
                        console.log(`Error GET_BCU_DISTRIBUTION_ID:`, e);
                        resolve({key: "DISTRIBUTION_ID", value: ""});
                    }
                });
            });
        },

        setSourceId: function(decoded){
            if (decoded.length <= 0) return;

            let result = JSON.parse(decoded) || {};

            console.log("GET_BCU_DISTRIBUTION_ID:", result);

            let sourceId = result[AvastWRC.Utils.getBrowserInfo().getCurrentBrowserShortName().toLowerCase()] || 
                           result[AvastWRC.Utils.getBrowserInfo().getCurrentBrowserShortName()] || 
                           "";

            let av = result.av || result.AV || "";

            if (sourceId === "" || av === "") return;

            if (AvastWRC.CONFIG.BRANDING_TYPE === AvastWRC.BRANDING_TYPE_AVAST && av.toLowerCase() === "avast" ||
                AvastWRC.CONFIG.BRANDING_TYPE === AvastWRC.BRANDING_TYPE_AVG && av.toLowerCase() === "avg") {
                AvastWRC.CONFIG.SOURCE_ID = sourceId;
            }
        },
...
(function(AvastWRC) {
	...
	AvastWRC.Utils.getClientInfoMessage = function(_clientInfo){
		...
		clientInfo.referer = cliInfo.referer;
